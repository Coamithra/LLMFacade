"""Live OpenAI tool roundtrip. Skips when ``OPENAI_API_KEY`` is unset."""
...
def test_openai_tool_roundtrip() -> None:
    llm = LLM()
    provider = llm.new_provider("openai")
    model = provider.new_model("gpt-4o-mini", max_tokens=512)

...
- **llama.cpp (`llamacpp`)**: Talks to a `llama-server` (or `llama-swap`) over its OpenAI-compat HTTP endpoint. No auth. Two modes, decided by the presence of `base_url` at provider construction:
  ...
  - **Autofit (`fit` / `fit_target` / `fit_ctx`, managed mode only)**: `fit` defaults to `True` and bakes `--fit on` into the rendered swap.yaml so newly registered models are OOM-safe by default — llama-server adjusts unset launch args to fit available VRAM at spawn time. `fit_target` is a tuple of per-device free-margin MiB targets (rendered as a comma-joined `--fit-target`); `fit_ctx` is the minimum context-size floor that `--fit` won't shrink past. Each `new_model()` also runs `llama-fit-params` once (if the binary is on PATH and `fit=True`), parses the printed estimate, and stashes it on the provider keyed by `model_id`. The conversation's JSONL settings header (and HTML log) then gains a top-level `fit_estimate` block of `{context_size, n_gpu_layers, parallel, est_vram_mib}` via the new `Provider.log_metadata(model_id=...)` hook. The block is labelled an *estimate*, not wire-truth — llama-server re-fits at spawn against current VRAM, which can drift between registration and first send. Subprocess failures (binary missing, non-zero exit, timeout, unparseable output) silently store `None` so logging never blocks `new_model()`. Note: enabling `fit=True` by default shifted the derived `model_id` hash on first run after the upgrade; bookkeeping only.

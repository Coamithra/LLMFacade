...
`SUPPORTS` carries three pure capability flags alongside the runtime knobs: `"tools"` (the provider can route a tool list — without it, `Conversation(tools=[...])` raises `UnsupportedFeature` at construction), `"tool_choice"` (forced selection beyond `"auto"` is supported), and `"vision"` (the provider can route `ImageBlock` content — see **Vision** under the **Capability gating** section above). All three are queryable via `is_available(...)` / `get_capabilities()` and narrowable per-model via `capability_override`. The flags are orthogonal: a provider may declare `"tools"` without `"tool_choice"` if its API has no forced-selection mode. For models that don't support a feature, callers narrow with e.g. `capability_override=provider.SUPPORTS - {"tools"}` (or `- {"vision"}`) on `new_model()`.
